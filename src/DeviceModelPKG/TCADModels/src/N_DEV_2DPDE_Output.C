...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_DEV_2DPDE_Output.C,v $
//
...
bool Instance::outputPlotFiles ()
{
  ...
  if (getSolverState().forceFinalOutput && getSolverState().currTime==lastOutputTime) skipOutput=true;
...
//
// Special Notes : It is somewhat easy to do this if we are running with
//                 the internal mesh, which is cartesian in appearance.
//                 If running without a cartesian mesh, then it is
//                 neccessary to do some interpolations.
//
// Scope         : public
// Creator       : Eric Keiter, SNL, Parallel Computational Sciences
...
bool Instance::outputTxtData ()
{
  ...
  if (!calcConductanceFlag)
  {
    ...
    fprintf(fp1, "%s","       This means that the condutances and capacitances\n");
