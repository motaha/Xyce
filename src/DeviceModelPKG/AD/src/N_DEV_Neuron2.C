...
template<>
ParametricData<Neuron2::Instance>::ParametricData()
{
  setNumNodes(2);
...
template<>
ParametricData<Neuron2::Model>::ParametricData()
{
                                          ...
                                          const vector<int> & extLIDVecRef)
{
  ...
  if (getDeviceOptions().debugLevel > 0 )
  {
    cout << "  li_Pos = " << li_Pos << endl
      ...
      << "  li_aPro = " << li_aPro << endl
      << "  li_bPro = " << li_bPro << endl
      << "  li_M_Pro = " << li_M_Pro << endl
      << "  li_H_Pro = " << li_H_Pro << endl
      << "  li_cPro = " << li_cPro << endl
      << "  li_CaPro = " << li_CaPro << endl;
  }
#endif
...
map<int,string> & Instance::getIntNameMap ()
{
  ...
  if (intNameMap.empty ())
  {
    ...
    tmpstr = getName() + "_" + "A";
    spiceInternalName (tmpstr);
    intNameMap[ li_aPro ] = tmpstr;

    tmpstr = getName() + "_" + "B";
    spiceInternalName (tmpstr);
    intNameMap[ li_bPro ] = tmpstr;

    tmpstr = getName() + "_" + "M_";
    spiceInternalName (tmpstr);
    intNameMap[ li_M_Pro ] = tmpstr;

    tmpstr = getName() + "_" + "H_";
    spiceInternalName (tmpstr);
    intNameMap[ li_H_Pro ] = tmpstr;

    tmpstr = getName() + "_" + "C";
    spiceInternalName (tmpstr);
    intNameMap[ li_cPro ] = tmpstr;

    tmpstr = getName() + "_" + "Ca";
    spiceInternalName (tmpstr);
    intNameMap[ li_CaPro ] = tmpstr;
...
std::ostream &Model::printOutInstances(std::ostream &os) const
{
  ...
  cout << endl;
  cout << "Number of Neuron instances: " << isize << endl;
  cout << "    name=\t\tmodelName\tParameters" << endl;
  for (i=0, iter=first; iter!=last; ++iter, ++i)
  {
    cout << "  " << i << ": " << (*iter)->getName() << "\t";
    cout << (*iter)->getModelName();
    cout << endl;
  }
