...
void N_TIA_OneStep::obtainPredictor()
{
  ...
  *ds.stoQCn0Ptr = *(ds.stoLeadCurrQCompHistory[0]);
  ...
  ds.stoQCpn0Ptr->putScalar(0.0); // need to verify that these are needed
  ...
  if (tiaParams.debugLevel > 1)
  {
    ...
    {
      cout << "\n stoHistory["<< i << "]: \n" << endl;
      (ds.stoHistory[i])->printPetraObject();
      cout << endl;
    }
    for (int i=0; i<=sec.currentOrder_ ; ++i)
    {
      cout << "\n stoLeadCurrQCompHistory["<< i << "]: \n" << endl;
      (ds.stoLeadCurrQCompHistory[i])->printPetraObject();
      cout << endl;
    }
    cout << "\n xn0: \n" << endl;
    	...
    	N_LAS_Vector * tmpSolVectorPtr, vector<N_LAS_Vector*> & historyVec)
{
  ...
  *tmpSolVectorPtr = *(historyVec[0]);
  ...
  {
    // do second order interpolation
    // X_interp = X + 0.5 * delta_t_requested * ((delta_X/delta_t)[last step] + (delta_X/delta_t)[last step - 1]) 
    //
    double dtr_step = 0.5 * dtr / sec.lastTimeStep;
    tmpSolVectorPtr->linearCombo(1.0,*tmpSolVectorPtr,dtr_step,*(historyVec[1]));
    dtr_step = 0.5 * dtr / timeStepForHistory2_;
    tmpSolVectorPtr->linearCombo(1.0,*tmpSolVectorPtr,dtr_step,*(historyVec[2]));
  }
    	...
    	N_LAS_Vector * tmpSolVectorPtr)
{
  ...
  string msg = "N_TIA_OneStep::interpolateMPDESolution: ";
        ...
        const std::vector<double> & fastTimes )
{
  ...
  {
    string msg = "N_TIA_OneStep::printMPDEOutputSolution: ";
        ...
        const int phiGID )
{
  ...
  if (blockTmpSolVectorPtr == NULL)
  {
    string msg = "N_TIA_OneStep::printWaMPDEOutputSolution: ";
    msg += "N_LAS_Vector ds.tmpSolVectorPtr is not of type N_LAS_BlockVector";
    N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::DEV_FATAL_0, msg);
    return(false);
  }
        ...
        bool skipPrintLineOutput)
{
  ...
  if (tiaParams.debugLevel > 0)
  {
    ...
    N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::USR_INFO_0, dashedline);
    N_ERH_ErrorMgr::report(N_ERH_ErrorMgr::USR_INFO_0,
      "  N_TIA_OneStep::printOutputSolution");
...
void N_TIA_OneStep::updateHistory()
{
  ...
  if (tiaParams.debugLevel > 1)
  {
   ...
   if (sec.currentOrder_ == 2)
   {
     ...
     *(ds.stoLeadCurrQCompHistory[2]) = *(ds.stoLeadCurrQCompHistory[1]);
   }
   ...
   (ds.stoLeadCurrQCompHistory[1])->linearCombo(1.0, *ds.nextStoreLeadCurrQCompPtr, -1.0,*(ds.stoLeadCurrQCompHistory[0]));
...
tiaParams.TimeStepLimitedbyBP = false;
  ...
  //      sec.psi_[i-1] = sec.psi_[i] - sec.currentTimeStep;
      ...
      {
        string msg = "N_TIA_OneStep::rejectStep: ";
...
//          sec.currentOrder_ = 1;
  ...
  {
    string tmp = "  OneStep:rejectStep: Warning: Local error test failed with constant step-size.\n";
...
{
  ds.nextStoreLeadCurrQCompDerivPtr->linearCombo(1.0,*ds.nextStoreLeadCurrQCompPtr,-1.0,*ds.stoQCn0Ptr);
  if (sec.currentOrder_ == 1)
  { 
  ...
  else 
  {
    ds.nextStoreLeadCurrQCompDerivPtr->
      linearCombo(-1.0,*(ds.currStoreLeadCurrQCompDerivPtr), 
      2.0/sec.currentTimeStep,*ds.nextStoreLeadCurrQCompDerivPtr);
  } 
