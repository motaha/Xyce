...
//-------------------------------------------------------------------------
// Filename       : $RCSfile: N_NLS_NOX_FastTests.C,v $
//
// Purpose        : Status testing designed to be similar to Spice style testing
//
// Special Notes  :
//
...
FastTests::FastTests(bool isTransient,
  ...
  XyceTests( isTransient, normF, machPrec, currSolVectorPtrPtr, epsilon_a, epsilon_r, 
		      tol, maxIters, convRate, relConvRate, maxConvRate, stagnationTol, maxBadSteps,
		      checkDeviceConvergence, smallUpdateTol, loader
#ifdef Xyce_NLS_MASKED_WRMS_NORMS
         , nonTrivialDeviceMaskFlag, maskVectorPtr
#endif		     
		     ),
  varTypeVec_( varTypeVec ), 
  voltZeroTol_( voltZeroTol ), 
  currZeroTol_( currZeroTol_ )
{
...
FastTests::checkStatus(const NOX::Solver::Generic& problem, NOX::StatusTest::CheckType checkType)
{
  ...
  const N_LAS_Vector& x = (dynamic_cast<const N_NLS_NOX::Vector&>(problem.getSolutionGroup().getX())).getNativeVectorRef();
  const N_LAS_Vector& oldX = (dynamic_cast<const N_NLS_NOX::Vector&>(problem.getPreviousSolutionGroup().getX())).getNativeVectorRef();
...
//   }
  ...
  if ((!isTransient_) || (niters_ == 0)) 
  {
    ...
    {
      //update[i] = x[i] - oldX[i];
      double localMax = Xycemax(fabs(x[i]), fabs(oldTimeStepX[i]));
      if( varTypeVec_[i] == 'I' )
      {
        if( localMax < currZeroTol_ )
        {
          weights[i] = N_UTL_MachineDependentParams::MachineBig();
        }
        else
        {
          weights[i] = epsilon_r_ * localMax + epsilon_a_;
        }
      }
      else
      {
        if( localMax < voltZeroTol_ )
        {
          weights[i] = N_UTL_MachineDependentParams::MachineBig();
        }
        else
...
std::ostream& FastTests::print(std::ostream& stream, int indent) const
{
  // precision
  int p = 5;
