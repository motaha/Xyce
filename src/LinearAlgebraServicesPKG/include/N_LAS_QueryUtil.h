...
public:
  ...
  virtual const vector<int> & rowList_GID() const = 0;
  virtual const vector< pair<int,int> > & rowList_ExternGID() const = 0;

  virtual const vector<int> & rowList_StateGID() const = 0;
  virtual const vector< pair<int,int> > & rowList_ExternStateGID() const = 0;

  virtual const vector<int> & rowList_StoreGID() const = 0;
  virtual const vector< pair<int,int> > & rowList_ExternStoreGID() const = 0;

  virtual const vector<int> & rowList_NumNZs() const = 0;

  virtual const vector< list<int> > & rowList_ColList() const = 0;

  virtual const vector<int> & vnodeGIDVec() const = 0;

  virtual const vector<int> & vsrcGIDVec() const = 0;

  //virtual const vector<int> & noDCPathGIDVec() const = 0;

  //virtual const vector<int> & connToOneTermGIDVec() const = 0;
  
  virtual const vector<char> & rowList_VarType() const = 0;
};
